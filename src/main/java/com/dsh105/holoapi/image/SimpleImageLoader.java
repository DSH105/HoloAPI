...
public class SimpleImageLoader implements ImageLoader<ImageGenerator> {
    ...
    public void loadImageConfiguration(YAMLConfig config) {
        ...
        URL_UNLOADED.clear();
        File imageFolder = new File(HoloAPI.getPlugin().getDataFolder() + File.separator + "images");
        ...
        if (cs != null) {
            for (String key : cs.getKeys(false)) {
                ...
                if (!EnumUtil.isEnumType(ImageLoader.ImageLoadType.class, imageType.toUpperCase())) {
                    HoloAPI.LOGGER.log(Level.INFO, "Failed to load image: " + key + ". Invalid image type.");
                    continue;
                }
        ...
        try {
            ...
            switch (type) {
                case URL:
                    ...
                    return null;
                case FILE:
                    File f = new File(HoloAPI.getPlugin().getDataFolder() + File.separator + "images" + File.separator + imagePath);
    ...
    private ImageGenerator prepareUrlGenerator(final CommandSender sender, final String key) {
        ...
        HoloAPI.LOGGER.log(Level.INFO, "Loading custom URL image of key " + key);
