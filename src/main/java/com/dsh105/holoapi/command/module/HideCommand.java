    ...
    public boolean onCommand(CommandSender sender, String[] args) {
        if (args.length == 3) {
            ...
            if (!hologram.getPlayerViews().keySet().contains(PlayerIdent.getIdentificationForAsString(target))) {
                Lang.sendTo(sender, Lang.HOLOGRAM_ALREADY_NOT_SEE.getValue().replace("%id%", args[1]).replace("%player%", args[2]));
                return true;
            }
            hologram.clear(target);
            Lang.sendTo(sender, Lang.HOLOGRAM_HIDE.getValue().replace("%id%", args[1]).replace("%player%", args[2]));
            return true;
        }
        return false;
    }
